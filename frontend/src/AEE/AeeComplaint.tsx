...
const AeeComplaint: React.FC = () => {
  ...
  const fetchComplaint = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.post(`http://localhost:4000/api/complaints/viewAeeCompalint/${id}`,{language:currentLanguage},{
          withCredentials:true
        });
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`http://localhost:4000/api/complaints/Aee/writeComment`, { 
        id:complaint._id,
        comment: newComment }, {
        withCredentials: true
      });
  ...
  const handleStatusChange = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`http://localhost:4000/api/complaints/Aee/changeStatus`, {id:complaint._id, status: newStatus }, {
        withCredentials: true
      });
      setComplaint(prevComplaint => {
        if (!prevComplaint) return null;
